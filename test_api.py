...
def test_get_imoveis_id(mock_connect_db, client):
    ...
    mock_connect_db.return_value = mock_conn
    response = client.get("/imoveis/id")
    assert response.status_code == 200
...
def test_add_imoveis(mock_connect_db, client):
    ...
    imovel = {"logradouro": 'Nicole Common', "tipo_logradouro": "Travessa", "bairro": "Lake Danielle", "cidade": "Judymouth", "cep": "00184", "tipo": "casa em condominio", "valor": 488423.52, "data_aquisicao": "2017-07-29"}
...
def test_update_imoveis(mock_connect_db, client):
    ...
    mock_connect_db.return_value = mock_conn
    response = client.get("/imoveis/id/update")
    assert response.status_code == 200

    expected_response = {
        "imoveis": [
            {'id': 1, "logradouro": 'Nicole Common', "tipo_logradouro": "Rua", "bairro": "Lake Danielle", "cidade": "Judymouth", "cep": "85184", "tipo": "casa em condominio", "valor": 488423.52, "data_aquisicao": "2017-07-29"},
        ]
    }
    assert response.get_json() == expected_response
...
def test_delete_imoveis(mock_connect_db, client):
    ...
    mock_connect_db.return_value = mock_conn
    response = client.get("/imoveis/id/delete")
    assert response.status_code == 200
...
def test_get_imoveis_tipo(mock_connect_db, client):
    ...
    mock_connect_db.return_value = mock_conn
    response = client.get("/imoveis/tipo_logradouro")
    assert response.status_code == 200
